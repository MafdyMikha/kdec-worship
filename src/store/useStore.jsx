...
export function AppProvider({ children }) {
  ...
  useEffect(() => {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange((event, session) => {
      if (event === 'PASSWORD_RECOVERY') {
        // User clicked the reset-password email link — show "set new password" screen,
        // do NOT route into the app even though Supabase creates a temp session
  ...
  const registerWithInvite = async (inviteCode, email, password, name) => {
    ...
    if (!inv) return { error: 'Invalid or expired invitation code' }
    if (new Date() > new Date(inv.expires_at)) return { error: 'This invitation has expired. Ask admin for a new one.' }

    // Sign up — emailRedirectTo is blank so no confirmation email is sent
    ...
    if (signInError) {
      // If Supabase still requires confirmation (dashboard setting), tell user clearly
      return { error: 'Account created! Please check your email to confirm, then sign in. Or ask your admin to disable email confirmation in Supabase.' }
    }
  ...
  const value = {
    isDemoMode, currentUser, authLoading, loading, isPasswordRecovery,
    people, songs, services, announcements, invitations, notifications,
    ROLES, POSITIONS,
    login, logout, registerWithInvite, forgotPassword, updatePassword, updateProfile,
